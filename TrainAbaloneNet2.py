...
from AbaloneNet2 import AbaloneNet, MCTS
...
def select_action(acts, probs, action_details):
    ...
    if isinstance(acts[0], int):  # If acts are indices
        valid_acts = [act for act in acts if act in action_details]
        actions = [action_details[act] for act in valid_acts]
        valid_probs = [p for act, p in zip(acts, probs) if act in action_details]
    else:
        actions = acts
        valid_probs = probs
    ...
    try:
        return np.random.choice(actions, p=valid_probs)
