...
class RewardSystemSimple:
    def __init__(self, player1, player2, push_off_reward=100, win_reward=1000, cluster_reward=5, 
                 isolation_reward=5, self_isolation_penalty = -2, threaten_reward=7, exposure_penalty=-3, multiple_threat_reward=160, push_reward = 10, 
                 max_moves = 300, repeated_move_penalty = -3):
        ...
        self.board_center = np.array([4, 4]) 
        self.off_board_distance = 10
    ...
    def is_cluster_improved(self, current_state, next_state):
        ...
        def get_ball_positions(grid, player):
            return [(i, j) for i in range(len(grid)) for j in range(len(grid[i])) if grid[i][j] == player]
        ...
        def hexagonal_distance(pos1, pos2):
            dx = pos1[0] - pos2[0]
            dy = pos1[1] - pos2[1]
        ...
        def evaluate_position(grid):
            white_positions = get_ball_positions(grid, 1)
            black_positions = get_ball_positions(grid, -1)

            white_com = calculate_center_of_mass(white_positions)
            black_com = calculate_center_of_mass(black_positions)

            # Weighted average of centers of mass and board center
            reference_point = 0.7 * (white_com + black_com) / 2 + 0.3 * self.board_center

            white_distance_sum = sum(hexagonal_distance(pos, reference_point) for pos in white_positions)
            black_distance_sum = sum(hexagonal_distance(pos, reference_point) for pos in black_positions)

            # Add penalty for balls pushed off the board
            white_distance_sum += (14 - len(white_positions)) * self.off_board_distance
            black_distance_sum += (14 - len(black_positions)) * self.off_board_distance

            # The score is the discrepancy between the two sums
            return black_distance_sum - white_distance_sum
        

        grid, current_player = current_state
        grid_next, _ = next_state

        current_score = evaluate_position(grid)
        next_score = evaluate_position(grid_next)

        # For the current player, a lower score is better
        if current_player == 1:  # White player
