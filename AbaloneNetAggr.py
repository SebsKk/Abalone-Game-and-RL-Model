...
class MCTSNode:
    ...
    def select(self, c_puct):
        return max(self.children.items(), 
                   key=lambda act_node: act_node[1].get_value(c_puct, act_node[0]))
    ...
    def get_value(self, c_puct, action, three_ball_bonus=2.5):
        # Determine if the move involves 3 balls
        is_three_ball_move = self.is_three_ball_move(action)

        # Calculate the exploration term (u)
        u = c_puct * self.P * np.sqrt(self.parent.n_visits) / (1 + self.n_visits)

        # Apply the bonus for moves with 3 balls
        if is_three_ball_move:
            
            u *= three_ball_bonus  # Increase exploration value for 3-ball moves

        return self.Q + u
    
    def is_three_ball_move(self, action):
...
class MCTS:
    ...
    def playout(self, state):
        ...
        while depth < max_depth:
            ...
            state, reward, done, ball_pushed_off = self.game_ops.step(action_dict)
            
            if ball_pushed_off == True:
                ball_pushed = True
    ...
    def get_move_probs(self, state, temp=1e-2):
        ...
        acts_with_visits = [act for act, visit in act_visits if visit > 0]

        if temp <= 1e-8:  # If temperature is virtually zero, use argmax
            print('Temperature is virtually zero. Using argmax.')
            ...
            total_visits = np.sum(visits[visited_mask])

            probs = np.zeros(len(self.game_ops.all_actions))
            visited_indices = np.where(visited_mask)[0]
            

            min_prob = 0.001
            assigned_probs = np.maximum(visits[visited_mask] / total_visits, min_prob)


            extra_prob = 1.0 - np.sum(assigned_probs)
            if extra_prob > 0:
                assigned_probs += (assigned_probs / np.sum(assigned_probs)) * extra_prob  # Distribute extra_prob proportionally


            for i, idx in enumerate(visited_indices):
                probs[idx] = assigned_probs[i]
                
        probs = probs / np.sum(probs)
        probabilies_that_are_not_zero = [prob for prob in probs if prob > 0]
