...
class AbaloneNet(nn.Module):
    ...
    def train(self, states, mcts_probs, winners):
        ...
        policy_loss = -torch.mean(torch.sum(mcts_probs_tensor * torch.log(policy), dim=1))
...
class MCTS:
    ...
    def is_game_over_for_playout(self, move_count):
        """Check if the game is over for playout purposes."""
        if any(player.score == 6 for player in self.game_ops.game.players):
            return True
        
        if move_count >= 400:  # Adjust this value as needed
            return True
    
        return False
    
    def playout(self, state, start_move_count):
        node = self.root
        path = []
        depth = 0
        ...
        while depth < max_depth:
            ...
            if done or self.is_game_over_for_playout(move_count):
                # print(f'Game over detected at depth {depth}. Move count: {move_count}. Final state: {state}')
                break
        ...
        if not self.is_game_over_for_playout(move_count):
            action_space, action_details, _ = self.game_ops.get_action_space()
        ...
        return path, leaf_value, move_count
    ...
    def get_move_probs(self, state, temp=1e-2):
        ...
        original_number_of_moves = self.game_ops.move_counter
        ...
        for _ in range(self.n_playout):
            ...
            self.game_ops.game.move_counter = original_number_of_moves

            print(f'number of original moves is {original_number_of_moves}')
            ...
            self.playout(copy.deepcopy(original_state), original_number_of_moves)
            #print('Playout done')
        ...
        if temp <= 1e-8:  # If temperature is virtually zero, use argmax
         ...
         # Create padded probabilities
        padded_probs = np.zeros(MAX_ACTIONS)
        for act, prob in zip(acts, probs):
            padded_probs[act] = prob

        # Create action mask
        action_mask = np.zeros(MAX_ACTIONS)
        action_mask[:len(action_space)] = 1

        # Normalize padded probabilities
        padded_probs = padded_probs / np.sum(padded_probs)

        return [action_details[act] for act in action_space], padded_probs, action_mask


    def pad_probabilities(probs, mask):
        
        MAX_ACTIONS = 140
        """Pad the probabilities to MAX_ACTIONS."""
        padded_probs = np.zeros(MAX_ACTIONS)
        padded_probs[:len(probs)] = probs
        padded_mask = np.zeros(MAX_ACTIONS)
        padded_mask[:len(mask)] = mask
        return padded_probs, padded_mask
