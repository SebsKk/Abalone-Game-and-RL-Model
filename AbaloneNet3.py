...
class MCTS:
    ...
    def _select_action(self, node, action_space, action_details):
        # Filter actions to only include those with non-None values
        valid_actions = [action for action in action_details.values() if action is not None]
        ...
        if random.random() < self.epsilon:
            ...
            return random.choice(valid_actions), node

        # Tree search (exploitation)
        while not node.is_leaf():
            action, child_node = node.select(self.c_puct)
            
            # Check if the action is in the valid set
            if isinstance(action, dict) and action in valid_actions:
                return action, child_node
            
            # Convert tuple action to dict and check if it's valid
            ...
            # If no valid action is found, continue down the tree
            node = child_node

        # If we've reached a leaf node, return a random valid action
        return random.choice(valid_actions), node
